...
![版本](https://img.shields.io/badge/版本-v2.7.0-800020)
...
### 🤖 AI 智能分析（豆包大模型）
配置 API Key 后，可一键生成五大板块综合分析报告（基金/投顾双模式）：

| 板块 | 内容 |
|------|------|
| **合规性分析** | 5 大维度逐条审查违规话术，原话 vs 优化话术对比表，整体合规评级 |
| **直播框架分析** | 时间线结构拆解、热门话题、**内容重复度分析**（话题频次表）、**标题质量评估**（5 维度打分） |
| **直播技巧优化** | 钩子话术、关注引导、预约引导的具体话术建议 |
| **直播质量评分** | 5 维度 × 20 分制量化评分，含**评论互动深度分析**（互动率、回复对照表、回复率汇总） |
| **整体优化建议** | 综合 4 板块按 P0-P3 优先级排序，Top 3 关键行动方案 |

- 支持 **一键生成综合报告**（4 板块并行 + 自动汇总）或展开单项单独生成
