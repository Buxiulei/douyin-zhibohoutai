...
前提说明：本直播间主播持有合法投顾资格证，因此对行情、板块进行分析推荐这一行为本身合规。但持证并不代表所有话术都合规，仍需严格检查话术内容是否存在以下违规风险：

一、通用合规检查：
- 绝对化用语（如"最好""第一""100%有效"等）
- 虚假宣传或夸大功效
- 诱导性话术（如虚假限时、虚假库存紧张）
- 涉及敏感领域的违规表述（医疗/教育相关承诺等）

二、证券类直播专项检查（即使持证也不允许）：
- 确定性收益承诺（如"这个票明天必涨""跟着我操作稳赚不赔""年化收益至少XX%"）
- 诱导跟单/跟买（如"赶紧上车""今天不买就来不及了""我已经重仓了你们跟上"）
- 缺少必要的风险提示（荐股/推荐板块时是否有配套的风险提示语）
- 承诺保本保收益
- 直播间公开荐个股代码（平台规范通常禁止在直播间公开讲解具体个股代码）

输出格式要求：
1. 先给出整体合规评级：🟢 低风险 / 🟡 中风险 / 🔴 高风险
2. 对每一条疑似违规话术，严格按以下表格对比呈现：
| 原始话术 | 风险类型 | 风险等级 | 优化话术 |
必须逐条列出，不可笼统概述。
3. 如未发现违规话术，需明确说明"本场直播未检测到明显合规风险"，并简要列出主播做得好的合规亮点。`
        },
        {
            key: 'framework',
            label: '直播框架分析',
